import React from "react";
import App from "../../components/App/App";

const wordList = [
  "vis",
  "toeter",
  "developer",
  "telefoon",
  "moeder",
  "snoer",
  "geeuw",
];

const maxGuesses = 5;

class AppContainer extends React.Component {
  state = {
    chosenWord: "",
    guessedLetters: [],
    currentChosenLetter: "",
    maxGuesses: maxGuesses,
  };

  chooseWord(list) {
    let index = Math.floor(Math.random() * list.length);
    console.log(`We chose word: ${list[index]}`);
    return list[index];
  }

  restartGameHandler = (event) => {
    this.setState({
      chosenWord: this.chooseWord(wordList),
      guessedLetters: [],
      currentChosenLetter: "",
    });
  };

  chosenLetterHandler = (event) => {
    const newState = { ...this.state };
    newState.currentChosenLetter = event.target.value;
    this.setState(newState);
  };

  guessLetterHandler = (event) => {
    const inputGiven = this.state.currentChosenLetter.length > 0;
    const newLetter = !this.state.guessedLetters.includes(
      this.state.currentChosenLetter
    );
    if (inputGiven && newLetter) {
      const newGuessedLetters = [...this.state.guessedLetters];
      newGuessedLetters.push(this.state.currentChosenLetter);
      this.setState({
        guessedLetters: newGuessedLetters,
      });
    }
    this.setState({ currentChosenLetter: "" });
    event.preventDefault();
  };
  // Problem: possible to guess the same letter
  // Tracing Problem: on submit the current chosen letter gets added to guessed letters
  // - App.js
  // 1. props.guessedLetters ["t", "t", "t"] after 3 x guessing "t"
  // 2. props.guessedLetterHandler: something not right with pushing currentChosenLetter > check function
  // 3. guessLetterHandler() pushes every input to the guessedLetters in state
  // Bug: r45-52 :
  // const inputGiven = this.state.currentChosenLetter.length > 0;
  //  if (inputGiven) {
  // current logic: if there is input, update state with input
  // expected logic: if there is input, and the input in not allready included in the guessed letters, then update state with input
  // Fix:
  // const inputGiven = this.state.currentChosenLetter.length > 0;
  // const newLetter = !this.state.guessedLetters.includes(this.state.currentChosenLetter);
  //  if (inputGiven && newLetter) {

  componentDidMount = () => {
    this.restartGameHandler();
  };

  render() {
    return (
      <App
        game={this.state}
        chosenLetterHandler={this.chosenLetterHandler}
        guessLetterHandler={this.guessLetterHandler}
        restartGameHandler={this.restartGameHandler}
      />
    );
  }
}

export default AppContainer;
